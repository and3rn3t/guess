...
async function maybeCaptureDataQualitySnapshot(
  ...
  try {
    const existing = await env.GUESS_DB.prepare(
      "SELECT COUNT(*) AS n FROM data_quality_snapshots WHERE captured_at >= unixepoch(date('now'))",
    ).first<{ n: number }>();
    if ((existing?.n ?? 0) > 0) return "skipped";

    const [chars, activeAttrs, attrRows, evidenceRows, agreement, disputes] =
      await Promise.all([
        env.GUESS_DB.prepare("SELECT COUNT(*) AS n FROM characters").first<{
          n: number;
        }>(),
        ...
        env.GUESS_DB.prepare(
          "SELECT COUNT(*) AS n FROM character_attributes",
        ).first<{ n: number }>(),
        env.GUESS_DB.prepare(
...
async function maybeBackfillDuplicateEmbeddings(
  env: AutomationEnv,
): Promise<number> {
  ...
  const request = new Request(
    "https://cron.local/api/admin/questions/duplicates/backfill",
    {
      method: "POST",
...
async function maybeKickEnrichment(
  env: AutomationEnv,
  batchId: string,
  ...
  try {
    // Skip if there is already an active (non-expired) enrich job
    const existing = await env.GUESS_DB.prepare(
      "SELECT id FROM enrich_job WHERE expires_at > unixepoch() LIMIT 1",
    ).first<{ id: number }>();
    if (existing) return "skipped";

    await runServerEnrichBatch(
      env as unknown as Parameters<typeof runServerEnrichBatch>[0],
      batchId,
    );
    return "started";
  } catch {
...
async function maybeMaterializeClosureQueue(
  ...
  if (!flagEnabled(env.AUTO_CLOSURE_QUEUE, true))
...
async function maybeMaterializeSourceHealth(
  ...
  if (!env.GUESS_DB) return { status: "skipped", report: null };
  if (!flagEnabled(env.AUTO_SOURCE_HEALTH, true))
    return { status: "skipped", report: null };

  const issueLimit = parseIntClamped(
    env.AUTO_SOURCE_HEALTH_LIMIT,
    200,
    1,
    1000,
  );
  ...
  try {
    const rowsResult = await env.GUESS_DB.prepare(
...
async function maybeAutoRetireQuestions(env: AutomationEnv): Promise<number> {
  ...
  for (const candidate of toRetire) {
    const result = await env.GUESS_DB.prepare(
      `UPDATE questions
...
export async function runAdminAutomation(
  ...
  if (!env.GUESS_DB) {
  ...
  } else {
    {
      const t0 = Date.now();
      try {
        summary.duplicatesEmbedded =
          await maybeBackfillDuplicateEmbeddings(env);
        if (summary.duplicatesEmbedded > 0) {
          summary.notes.push(
            `duplicate embeddings backfilled: ${summary.duplicatesEmbedded}`,
          );
        }
      } catch (err) {
...

    {
      const t0 = Date.now();
      try {
        summary.enrichmentKick = await maybeKickEnrichment(
          env,
          `cron-${trigger.scheduledTime}`,
        );
        if (summary.enrichmentKick === "error") {
          summary.errorCount += 1;
          summary.stepErrors.enrichment =
            "enrichment run returned error status";
...

    {
      ...
      try {
        ...
        if (closure.status === "error") {
          summary.errorCount += 1;
          summary.stepErrors.closureQueue =
            "closure queue run returned error status";
...

    {
      ...
      try {
        ...
        if (sourceHealth.status === "error") {
          summary.errorCount += 1;
          summary.stepErrors.sourceHealth =
            "source health run returned error status";
      ...
      try {
        const closureForSnapshot =
          summary.closureQueue.status === "generated"
            ...
            : undefined;
        summary.snapshot = await maybeCaptureDataQualitySnapshot(
          env,
          closureForSnapshot,
        );
...

    {
      ...
      try {
        ...
        if (summary.retiredQuestions > 0) {
          summary.notes.push(
            `questions auto-retired: ${summary.retiredQuestions}`,
          );
        }
      } catch (err) {
        summary.errorCount += 1;
