...
import {
  ...
  loadQuestionsWithRetirementFallback,
  parseTrivia,
} from "./_start_helpers";
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    // Check questions D1 cache first — questions are immutable at runtime, so a 1h
    ...
    if (cachedQuestions) {
      ...
      if (serverQuestions.length > 0) {
        context.waitUntil(storeCachedQuestions(db, serverQuestions));
      }
    }
    ...
    for (const q of serverQuestions) {
      const known = serverChars.filter(
    ...
    //   1. Rephrase first question via LLM (with D1 cache for frequently-seen first questions)
    //   2. Store session in D1 (required before any answer can be processed)
    ...
    // with stamped variant + selector + userId.
    const assignment = await assignVariant(db, userId);
    session.variant = assignment.variant;
    session.selector = assignment.selector;
    session.userId = userId;
    context.waitUntil(storeSession(db, session));

    // D1 game_sessions row for history/analytics
    context.waitUntil(
      d1Run(
        db,
