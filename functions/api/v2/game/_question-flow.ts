...
export function buildNextQuestionResponse(input: {
  ...
  const eliminated = calculateEliminatedCount(
    input.session,
...
export function applyAnswerAndFilter(
  ...
  const askedQuestion = session.currentQuestion as NonNullable<
...
export async function advanceToNextQuestion(
  ...
  const { env, db, session, nextQuestion, reasoning, questionNumber } = input;

  const questionLookup = new Map(
    session.questions.map((q) => [q.attribute, q.text]),
  );
...
export async function persistAndSyncAnswerTurn(
  input: PersistAndSyncAnswerTurnInput,
): Promise<void> {
