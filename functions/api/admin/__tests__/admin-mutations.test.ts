...
import {
  onRequestPatch as questionsPatch,
} from '../questions/[key]/index'
...
describe('DELETE /api/admin/error-logs', () => {
  it('clears all rows when no `before` param is supplied', async () => {
    db.raw
      .prepare(`INSERT INTO error_logs (level, source, message) VALUES (?, ?, ?)`)
      .run('error', 'test', 'boom')
    db.raw
      .prepare(`INSERT INTO error_logs (level, source, message) VALUES (?, ?, ?)`)
      .run('warn', 'test', 'meh')
...
describe('POST /api/admin/characters/:id/validate', () => {
  it('returns parsed LLM issues when OpenAI is configured', async () => {
    ...
    try {
      const res = await invokeHandler<{ issues: Array<{ attributeKey: string }> }>(
        charactersValidate,
        {
          params: { id: 'mario' },
...
describe('POST /api/admin/proposed-attributes', () => {
  ...
  it('skips entries missing required fields', async () => {
    const res = await invokeHandler<{ inserted: number; submitted: number }>(proposedPost, {
      body: {
        proposals: [
...
describe('POST /api/admin/proposed-attributes/:id', () => {
  it('approves a pending proposal and creates an attribute_definitions row', async () => {
    ...
    const res = await invokeHandler<{ ok: boolean; action: string; key: string }>(
      proposedReview,
      {
        params: { id: String(id) },
        body: { action: 'approve' },
    ...
    expect(res.status).toBe(200)
    expect(res.body).toMatchObject({ ok: true, action: 'approved', key: 'isHero' })
...
describe('POST /api/admin/proposed-attributes/:id/score', () => {
  it('returns clamped LLM score', async () => {
    ...
    try {
      const res = await invokeHandler<{ score: number; concerns: string[] }>(proposedScore, {
        params: { id: '1' },
...
describe('PATCH /api/admin/attribute-disputes', () => {
  it('marks a dispute resolved', async () => {
    ...
    expect(res.status).toBe(200)
    const row = db.raw
      .prepare(`SELECT status, resolved_by FROM attribute_disputes WHERE id = ?`)
...
describe('POST /api/admin/attribute-disputes-ai', () => {
  it('returns parsed verdict from LLM', async () => {
    ...
    try {
      const res = await invokeHandler<{ correct: string; confidence: number }>(disputesAi, {
        body: {
          characterName: 'Mario',
          attributeKey: 'wearsHat',
          currentValue: false,
...
describe('POST /api/admin/community', () => {
  it('dismisses corrections by deleting the KV key', async () => {
    await kv.put('corrections:mario', JSON.stringify([{ attribute: 'wearsHat', suggestedValue: true }]))
    const res = await invokeHandler(communityPost, {
      body: { action: 'dismiss', characterId: 'mario' },
      env: buildEnv({ db, kv }),
    })
    expect(res.status).toBe(200)
    expect(await kv.get('corrections:mario')).toBeNull()
  })

  it('applies majority votes to character_attributes', async () => {
    seedCharacter(db, 'mario')
    seedAttributeDefinition(db, 'wearsHat')
    await kv.put(
      'corrections:mario',
      JSON.stringify([
        { attribute: 'wearsHat', suggestedValue: true, userId: 'u1', currentValue: null, createdAt: 1 },
        { attribute: 'wearsHat', suggestedValue: true, userId: 'u2', currentValue: null, createdAt: 2 },
      ]),
    )
    const res = await invokeHandler<{ applied: number }>(communityPost, {
      body: { action: 'apply', characterId: 'mario' },
      env: buildEnv({ db, kv }),
    })
    expect(res.status).toBe(200)
    ...
    const row = db.raw
      ...
      .get('mario', 'wearsHat') as { value: number; evidence: string }
    expect(row.value).toBe(1)
    expect(row.evidence).toMatch(/^community:vote:/)
    expect(await kv.get('corrections:mario')).toBeNull()
  })
...
describe('POST /api/admin/coverage-priority', () => {
  it('returns empty list when no sparse attributes exist (skips LLM)', async () => {
    ...
    try {
      const res = await invokeHandler<{ items: unknown[] }>(coveragePriorityPost, {
        env: buildEnv({ db, kv, openaiKey: 'k' }),
      })
      expect(res.status).toBe(200)
...
describe('POST /api/admin/enrich/start', () => {
  ...
  it('clears the signal on stop', async () => {
    await kv.put('admin:enrich-start', '{}')
    const res = await invokeHandler(enrichStartPost, {
      body: { action: 'stop' },
      env: buildEnv({ db, kv }),
    })
...
describe('POST /api/admin/enrichment', () => {
  it('queues a full retry when no IDs are supplied', async () => {
    ...
    expect(await kv.get('admin:enrich-retry')).toBeTruthy()
  })
  ...
  it('queues specific IDs when provided', async () => {
    ...
    expect(await kv.get('admin:enrich-retry-ids')).toBeTruthy()
  })
})
...
describe('POST /api/admin/experiments', () => {
  it('updates pct, selector, and autoTuneEnabled', async () => {
    ...
    expect(await kv.get('ab:experiment-pct')).toBe('25')
    expect(await kv.get('ab:experiment-selector')).toBe('mcts')
    expect(await kv.get('engine:auto-tune-enabled')).toBe('true')
  })
...
describe('POST /api/admin/analytics/insights', () => {
  it('caches LLM result in KV', async () => {
    const stub = mockOpenAi({ content: 'three insights here' })
    try {
      const res = await invokeHandler<{ insights: string }>(analyticsInsightsPost, {
        body: { summary: [{ event_type: 'play', count: 5 }], totalGames7d: 5 },
        env: buildEnv({ db, kv, openaiKey: 'k' }),
      })
      expect(res.status).toBe(200)
      expect(res.body.insights).toContain('three insights')
      expect(await kv.get('admin:analytics-insights')).toBeTruthy()
      expect(stub.calls).toHaveLength(1)
    } finally {
      stub.restore()
    }
  ...
  it('serves from cache on second call', async () => {
    await kv.put('admin:analytics-insights', JSON.stringify({ insights: 'cached', generated_at: 1 }))
...
describe('POST /api/admin/pipeline', () => {
  ...
  it('rejects an invalid step', async () => {
    const res = await invokeHandler(pipelinePost, {
      body: { runBatch: 'b1', characterId: 'm', step: 'nope', status: 'success' },
      env: buildEnv({ db }),
    })
    expect(res.status).toBe(400)
  })
...
describe('POST /api/admin/questions/:key/score', () => {
  it('returns clamped LLM score', async () => {
    const stub = mockOpenAi({
      content: JSON.stringify({ clarity: 9, power: 0, grammar: 4, rewrite: 'Better?' }),
    })
    try {
      const res = await invokeHandler<{ clarity: number; power: number; grammar: number }>(
        questionsScore,
        {
          params: { key: 'k' },
...
describe('POST /api/admin/resolve-stack', () => {
  ...
  it('returns frames with `null` resolution when the map is missing', async () => {
    const r2 = createTestR2()
    await kv.put('deploy:current-sha', 'abc1234')
    const res = await invokeHandler<{ frames: Array<{ resolved: unknown; reason?: string }> }>(
      resolveStackPost,
      {
        body: { stack: 'at fn (https://x/assets/index-abc.js:42:13)' },
        env: buildEnv({ db, kv, r2 }),
      },
    )
    expect(res.status).toBe(200)
...
describe('POST /api/admin/upload-attrs', () => {
  ...
  it('returns 403 when secret is wrong', async () => {
    await kv.put('admin:secret', 'right')
    const res = await invokeHandler(uploadAttrsPost, {
      body: { secret: 'wrong' },
      env: buildEnv({ db, kv }),
    })
  ...
  it('rejects oversized batches', async () => {
    await kv.put('admin:secret', 'right')
    const huge = Array.from({ length: 501 }, (_, i) => ({ c: `c${i}`, k: 'k', v: 1 }))
    const res = await invokeHandler(uploadAttrsPost, {
...
function rowCount(table: string, where = '1=1'): number {
  const row = db.raw.prepare(`SELECT COUNT(*) as n FROM ${table} WHERE ${where}`).get() as {
    n: number
  }
  return row.n
}
