...
describe('buildUserPrompt', () => {
  it('lists characters with name and category', () => {
    const prompt = buildUserPrompt([
      { id: 'mario', name: 'Mario', category: 'video-games', description: null },
      { id: 'gandalf', name: 'Gandalf', category: 'fantasy', description: 'A wizard' },
    ])
  ...
  it('truncates long descriptions at 200 chars', () => {
    ...
    const prompt = buildUserPrompt([{ id: 'a', name: 'A', category: 'anime', description: longDesc }])
...
describe('parseOpenAIContent', () => {
  it('extracts valid true/false/null values for known keys', () => {
    ...
    const result = parseOpenAIContent(content, ['mario'], new Set(['isHero', 'isVillain', 'hasMagic']))
    expect(result).toEqual({ mario: { isHero: true, isVillain: false, hasMagic: null } })
  })
  ...
  it('returns empty object on invalid JSON', () => {
    const result = parseOpenAIContent('{bad json', ['mario'], new Set(['isHero']))
    expect(result).toEqual({})
  })

  it('skips characters missing from the response', () => {
    const content = JSON.stringify({ mario: { isHero: true } })
    const result = parseOpenAIContent(content, ['mario', 'luigi'], new Set(['isHero']))
...
describe('runServerEnrichBatch', () => {
  ...
  it('writes character_attributes and marks pipeline_runs success on valid LLM response', async () => {
    ...
    const llmContent = JSON.stringify({ mario: { isHero: true, isVillain: false } })
    const { restore } = mockOpenAi({ content: llmContent })
    ...
    const attrs = db.raw
      .prepare(`SELECT attribute_key, value FROM character_attributes WHERE character_id = 'mario' ORDER BY attribute_key`)
    ...
    const run = db.raw
      .prepare(`SELECT status FROM pipeline_runs WHERE character_id = 'mario' AND step = 'enrich'`)
  ...
  it('writes evidence tag on character_attributes rows', async () => {
    ...
    const row = db.raw
      .prepare(`SELECT evidence FROM character_attributes WHERE character_id = 'mario' AND attribute_key = 'isHero'`)
  ...
  it('marks pipeline_runs as error and clears KV on LLM fetch failure', async () => {
    ...
    const { restore } = mockOpenAi({ status: 500, body: 'Internal Server Error' })
    ...
    const run = db.raw
      .prepare(`SELECT status, error FROM pipeline_runs WHERE character_id = 'luigi' AND step = 'enrich'`)
    ...
    const attrs = db.raw
      .prepare(`SELECT COUNT(*) AS n FROM character_attributes WHERE character_id = 'luigi'`)
      .get() as { n: number }
  ...
  it('marks pipeline_runs as error when character is missing from LLM response', async () => {
    ...
    const run = db.raw
      .prepare(`SELECT status FROM pipeline_runs WHERE character_id = 'wario' AND step = 'enrich'`)
      .get() as { status: string } | undefined
    expect(run?.status).toBe('error')
  })
