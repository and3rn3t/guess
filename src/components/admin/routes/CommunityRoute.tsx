...
export default function CommunityRoute(): React.JSX.Element {
  ...
  const sendAction = async (
    ...
    try {
      ...
      if (!res.ok) throw new Error(`${res.status}`);
      toast.success(action === "apply" ? "Corrections applied" : "Dismissed");
      await fetchData();
    } catch (e) {
      ...
      {data?.total === 0 && !loading && (
        <div className="rounded-xl border bg-card px-6 py-12 text-center space-y-3">
          ...
          <p className="text-muted-foreground text-sm">
            No pending community corrections. Corrections are auto-applied at{" "}
            {3} votes — anything remaining here hasn't reached the threshold
            yet.
          </p>
        </div>
      )}
        ...
        {(data?.items ?? []).map((item) => (
          ...
          >
            <button
              ...
              onClick={() =>
                setExpanded((e) =>
                  e === item.characterId ? null : item.characterId,
                )
              }
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      void sendAction(item.characterId, "apply");
                    }}
                  >
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      void sendAction(item.characterId, "dismiss");
                    }}
                  >
                  ...
                  <tbody className="divide-y divide-border">
                    {item.attributes.map((a) => (
                      <tr key={a.attribute}>
                        ...
                        <td className="py-2 text-center text-green-400 text-xs">
                          {a.yesVotes}
                        </td>
                        <td className="py-2 text-center text-red-400 text-xs">
                          {a.noVotes}
                        </td>
