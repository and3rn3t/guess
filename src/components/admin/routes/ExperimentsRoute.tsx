        ...
        <>
          <section className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div className="rounded-lg border border-border/60 bg-card p-4">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {control
                  ? `${control.games.toLocaleString()} games · ±${(control.ci95 * 100).toFixed(1)}%`
                  : "no control data"}
              </p>
            </div>
          </section>
          ...
          <section className="rounded-lg border border-border/60 bg-card overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {data.arms.map((arm) => {
                  const delta =
                    control && arm.variant !== "control"
                      ? arm.winRate - control.winRate
                      : null;
                  return (
                    <tr
                      key={`${arm.variant}-${arm.selector}`}
                      className="border-t border-border/60"
                    >
                      <td className="px-4 py-2 font-medium">{arm.variant}</td>
                      <td className="px-4 py-2">{arm.selector}</td>
                      <td className="px-4 py-2 text-right tabular-nums">
                        {arm.games.toLocaleString()}
                      </td>
                      <td className="px-4 py-2 text-right tabular-nums">
                        {pct(arm.winRate)}{" "}
                        <span className="text-muted-foreground text-xs">
                          ±{(arm.ci95 * 100).toFixed(1)}%
                        </span>
                      </td>
                      <td className="px-4 py-2 text-right tabular-nums">
                        {fmtNum(arm.avgQuestions, 1)}
                      </td>
                      <td className="px-4 py-2 text-right tabular-nums">
                        {fmtNum(arm.avgConfidence, 2)}
                      </td>
                      <td className="px-4 py-2 text-right tabular-nums">
                        {delta === null ? (
          ...
          <section className="rounded-lg border border-border/60 bg-card p-4 space-y-2 text-sm">
            ...
            <p className="text-xs text-muted-foreground/80 pt-2">
              The weekly{" "}
              <code className="rounded bg-muted px-1">engine-self-tune</code>{" "}
              workflow promotes{" "}
              ...
              automatically when these criteria are met.
            </p>
          </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <label className="flex flex-col gap-1">
                ...
                <input
                  ...
                  onBlur={(e) => {
                    ...
                    if (Number.isFinite(v) && v !== data.config.pct)
                      void updateConfig({ pct: v });
                  }}
              ...
              <label className="flex flex-col gap-1">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (v !== data.config.selector)
                      void updateConfig({ selector: v });
                  }}
              ...
              <label className="flex flex-col gap-1">
                ...
                <button
                  ...
                  onClick={() =>
                    void updateConfig({
                      autoTuneEnabled: !data.config.autoTuneEnabled,
                    })
                  }
                ...
                >
                  {data.config.autoTuneEnabled ? "ENABLED" : "DISABLED"} — click
                  to toggle
                </button>
              </label>
            </div>
