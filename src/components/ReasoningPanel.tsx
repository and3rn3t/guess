...
function ReasoningPanelBase({ reasoning, readiness = null, isThinking = false, streamComment = '', isStreamingComment = false, confidenceHistory = [] }: Readonly<ReasoningPanelProps>) {
  ...
  if (!reasoning) {
    if (isThinking) {
      // Skeleton matching the loaded layout: stats grid + 3 candidate rows.
      // Same outer dimensions as the populated card so layout doesn't shift
      // when the first reasoning payload arrives.
      return (
        <Card className="p-6 bg-card/50 backdrop-blur-sm border-2 border-primary/20" aria-busy="true">
          <div className="flex items-center gap-3 mb-4">
            <Brain className="text-accent" size={24} weight="duotone" />
            <h3 className="text-xl font-semibold">AI Reasoning</h3>
          </div>
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div className="h-16 rounded-lg bg-secondary/20 border border-secondary animate-pulse" />
            <div className="h-16 rounded-lg bg-secondary/20 border border-secondary animate-pulse" />
          </div>
          <div className="space-y-2">
            <div className="h-6 w-1/3 rounded bg-accent/10 animate-pulse" />
            {[0, 1, 2].map((i) => (
              <div
                key={i}
                className={`h-8 rounded bg-accent/5 animate-pulse skeleton-row-${i}`}
              />
            ))}
          </div>
        </Card>
