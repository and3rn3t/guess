...
- ⬜ **Next:** [RF.v2.5](#rfv2-5) (admin API client consolidation — `adminApi.ts` + `useCharactersListing.ts` + `LiveOpsContext` + `AdminShell` badges + `FunnelRoute` + `SlaTargetsPanel` + `CuratorQueuePanel` + `ImageHealthPanel` + `MatrixRoute` + `ConfusionRoute` on generated types; ~12 admin route bare-fetch call sites remain to sweep).
