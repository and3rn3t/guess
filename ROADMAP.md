...
| <a id="dx-31"></a>DX.31 | **OpenAPI spec auto-derived from handlers** | Medium | 🟡 2026-05-03 — Deterministic artifact generation is in place (`docs/openapi.{json,yaml}`, `docs/openapi-inventory.json`, `public/openapi.yaml`) with validate/drift scripts and `/admin/api-docs` scaffold. Coverage extended beyond core game routes: explicit request/response schemas now include v2 attributes/characters/daily/leaderboard/events/history/questions/stats/game-reveal and admin `/api/admin/about`; inventory now reflects 82 endpoints. Next: continue replacing generic-object response schemas across remaining admin surfaces. |
