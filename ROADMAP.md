...
- 🟡 **In progress:** [DQ.33](#dq-33) deterministic null-closure queue
...
| <a id="dx-23"></a>DX.23 | **`pnpm doctor` health check** | Low | 🟡 2026-05-03 — `scripts/doctor.ts` checks Node ≥ 22, pnpm version, `wrangler whoami`, `.dev.vars` + `.env` required keys, `node_modules`, gitleaks (warn). `.dev.vars.example` added as source-of-truth for required keys. `ARCHITECTURE.md` → Local Dev Shell Setup section added. |
...
| <a id="dx-38"></a>DX.38 | **Conventional commit linter** | Low | 🟡 2026-05-03 — `commitlint.config.mjs` + `.husky/commit-msg` hook with shell guard that skips Merge/Revert/fixup!/squash! auto-messages so rebases are never blocked. `@commitlint/cli` + `@commitlint/config-conventional` added as devDeps. |
