...
| # | Item | Effort | Notes |
|---|------|--------|-------|
| <a id="dq-31"></a>DQ.31 | **Definition of complete + release gate** | Medium | Introduce a canonical `data_complete_score` and hard thresholds for production readiness. Minimum gate: global completeness %, per-category completeness %, evidence coverage %, source-id coverage %, and unresolved-high-priority dispute count. Start warn-only for 2 weeks, then fail release checks when below threshold. |
| <a id="dq-32"></a>DQ.32 | **Attribute completeness SLA matrix** | Medium | Add a committed matrix (`data/attribute-completeness-sla.json`) with per-(attribute, category) targets (example: `isHuman` 100%, `firstAppearedYear` 95%, `personality` 70%). Dashboard and cron checks read from one source of truth. |
| <a id="dq-33"></a>DQ.33 | **Deterministic null-closure queue** | Medium | Build a daily queue of missing (character, attribute) pairs prioritized by `popularity × selector_impact × confidence_gap × staleness`. This is the canonical feed for DQ.22 auto-fill and manual review, so high-impact NULLs close first. |
| <a id="dq-34"></a>DQ.34 | **Source-ID completeness guardrail** | Low | Track `% characters with valid source IDs` for TMDb, AniList, IGDB, ComicVine, Wikidata. Daily job verifies referential health (missing, malformed, 404, rate-limited). Broken IDs route to admin repair queue before they silently degrade enrichment quality. |
| <a id="dq-35"></a>DQ.35 | **Image health completeness guardrail** | Low | Track `% characters with usable portrait` (present, fetchable, non-placeholder, quality above threshold, style not mismatched). Failed rows go to an image-fix queue; DQ.12 audits and EN.20 replacement jobs consume this queue. |
| <a id="dq-36"></a>DQ.36 | **Manual curator closure queue** | Medium | Introduce a first-class queue for ambiguous/uninferable rows (`cannot_infer`, `canon_conflict`, `subjective`) with assignment, resolution reason, and lock semantics. Prevents endless reprocessing loops for cases automation cannot resolve safely. |
| <a id="dq-37"></a>DQ.37 | **Risk-tiered revalidation cadence** | Low | Revalidate characters by risk tier instead of flat sampling: Tier 1 (top played + high disagreement) daily, Tier 2 weekly, Tier 3 monthly. Maximizes quality impact per token dollar while improving coverage on the rows players actually see. |
| <a id="dq-38"></a>DQ.38 | **Completeness burndown + weekly report** | Low | Add `/admin/data-quality/completeness` burndown for NULL backlog, SLA misses, and queue aging; emit a weekly markdown artifact (`data/quality-reports/quality-YYYY-WW.md`) summarizing closure velocity, blockers, and next highest-impact gaps. |
...
**Data completeness push (next highest leverage):** DQ.31 definition-of-complete gate · DQ.32 SLA matrix · DQ.33 null-closure queue · DQ.34 source-ID completeness · DQ.35 image health completeness · DQ.36 curator closure queue · DQ.38 weekly burndown report
