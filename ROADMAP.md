...
- ⬜ **Next:** [RF.v2.5](#rfv2-5) (admin API client consolidation — `adminApi.ts` + `useCharactersListing.ts` + `LiveOpsContext` + `AdminShell` badges + `FunnelRoute` + `SlaTargetsPanel` + `CuratorQueuePanel` + `ImageHealthPanel` + `MatrixRoute` + `ConfusionRoute` + `AnalyticsRoute` + `EnrichmentRoute` + `AboutRoute` + `StressTestRoute` + `EnrichDashboardRoute` on generated types; ~6 admin route bare-fetch call sites remain to sweep).
...
**Status:** 🟡 in progress (writer + flag shipped 2026-05-26; awaiting operator deploy + flag flip)
...
- [x] `logError` emits `console.error(JSON.stringify({kind:'guess_error_event',…}))` instead of touching D1 when `ERROR_LOG_VIA_TAIL` is set. Feature-flagged so the flip is reversible.
- [x] Tail Worker parses `event.logs` for `guess_error_event` entries and batch-inserts into `error_logs` via a `GUESS_DB` D1 binding ([tail-worker/src/_error_log_writer.ts](tail-worker/src/_error_log_writer.ts), [tail-worker/src/_error_log_writer.test.ts](tail-worker/src/_error_log_writer.test.ts)).
- [x] Tail-worker `wrangler.toml` declares the `GUESS_DB` binding (production + preview).
- [x] All request-path `logError(...)` call sites in `functions/**` pass `context.env` so the flag activates uniformly.
- [ ] **Operator step:** deploy Tail Worker (`wrangler deploy --config tail-worker/wrangler.toml --env production`), verify rows in `error_logs` via `/admin/error-logs`, then set `ERROR_LOG_VIA_TAIL=1` in `[env.production.vars]`.
- [ ] After ≥7d of clean dual-stream observation, remove the D1 write path from `logError` (drop the `D1Database` overload).
...
| 2026-05-26 | PI.3.b shipped in two halves: code-ready (writer + flag + D1 binding + all 22 request-path `logError` call sites migrated) lands now; operator-flip (Tail Worker deploy → verify → `ERROR_LOG_VIA_TAIL=1`) deferred to a separate non-code change. After ≥7d of clean dual-stream observation, the D1 write path will be removed from `logError` in a follow-up. | The roadmap's coordinated-deploy risk was the only thing keeping PI.3.b off the active batch. Splitting into ship-code-now / flip-flag-later keeps the diff reviewable as a pure feature-flag add (legacy path untouched by default), preserves rollback (flip flag back to drop envelopes), and lets the operator schedule the deploy without blocking other roadmap progress. Migrating all 22 `logError` sites in one mechanical pass (via perl in-place s/) was a one-time cost paid up front so the flag flip itself is a single env-var change. |
