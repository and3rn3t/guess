...
- 🟡 **In progress:** [DX.31](#dx-31) OpenAPI spec auto-derived from handlers
- ▶ **Up next:** (none)
...
- 🟡 **In progress:** [I.9](#i-9) AI Gateway semantic caching 7d cache-hit window (expected: 2026-05-07) · [DQ.33](#dq-33) deterministic null-closure queue · [DQ.36](#dq-36) manual curator closure queue · [DX.31](#dx-31) OpenAPI spec auto-derived from handlers
- ▶ **Up next:** (none)
...
| <a id="dx-31"></a>DX.31 | **OpenAPI spec auto-derived from handlers** | Medium | 🟡 2026-05-03 — Deterministic artifact generation is in place (`docs/openapi.{json,yaml}`, `docs/openapi-inventory.json`, `public/openapi.yaml`) with validate/drift scripts and `/admin/api-docs` scaffold. Coverage now includes explicit request/response schemas for v2 attributes/characters/daily/leaderboard/events/history/questions/stats/game-reveal plus admin `about`, `analytics`, `analytics/aha-moments`, `analytics/insights`, `attribute-disputes`, `attribute-disputes-ai`, `automation-status`, `characters`, `characters/{id}` (GET/PATCH/DELETE), `characters/{id}/validate`, `community` (GET/POST), `confusion`, `costs`, `coverage`, `coverage-priority` (POST), `curator-queue`, `dashboard`, `data-quality`, `data-quality-sla`, `error-logs`, `experiments` (GET/POST), `funnel`, `image-health`, `live-ops`, `matrix`, `pipeline`, `proposed-attributes` (GET/POST/PATCH), `proposed-attributes/{id}` (POST), `proposed-attributes/{id}/score`, `questions`, `questions/{key}` (PATCH), `questions/{key}/retire`, `questions/{key}/score`, `questions/{key}/unretire`, `questions/bulk`, `questions/expand`, `questions/retirement-queue`, `source-health`, `source-health-status`, `triage`, and `workflow-progress`; inventory reflects 82 endpoints. Remaining generic admin contracts: 18. Next: continue replacing remaining generic-object contracts on long-tail admin routes. |
