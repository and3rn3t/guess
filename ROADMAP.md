...
| <a id="dx-31"></a>DX.31 | **OpenAPI spec auto-derived from handlers** | Medium | 🟡 2026-05-03 — Deterministic artifact generation is in place (`docs/openapi.{json,yaml}`, `docs/openapi-inventory.json`, `public/openapi.yaml`) with validate/drift scripts and `/admin/api-docs` scaffold. Coverage now includes explicit request/response schemas for v2 attributes/characters/daily/leaderboard/events/history/questions/stats/game-reveal plus admin `about`, `automation-status`, `curator-queue`, `data-quality-sla`, `image-health`, `source-health`, and `source-health-status`; inventory reflects 82 endpoints. Next: continue replacing remaining generic-object contracts on long-tail admin routes. |
