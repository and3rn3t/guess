...
| <a id="dx-31"></a>DX.31 | **OpenAPI spec auto-derived from handlers** | Medium | 🟡 2026-05-03 — Deterministic artifact generation is in place (`docs/openapi.{json,yaml}`, `docs/openapi-inventory.json`, `public/openapi.yaml`) with validate/drift scripts and `/admin/api-docs` scaffold. Coverage now includes explicit request/response schemas for v2 attributes/characters/daily/leaderboard/events/history/questions/stats/game-reveal plus all discovered admin operations, including long-tail routes: `community`, `coverage-priority`, `data-quality/closure-queue`, `data-quality/closure-queue-status`, `enrich/start`, `enrich/stream`, `enrichment`, `error-logs` (DELETE), `experiments`, `hygiene-attributes`, `hygiene-categories`, `hygiene-duplicates`, `hygiene-question-scores`, `questions/duplicates/{backfill,dismiss,merge}`, `recommender`, `resolve-stack`, `stress-test`, and `upload-attrs`; inventory reflects 82 endpoints. Remaining generic admin contracts: 0. Remaining generic contracts are legacy `/api/*` routes (`characters`, `corrections`, `csp-report`, `images`, `llm`, `llm-stream`, `questions`, `stats`, `sync`) with 14 generic responses total. Next: decide whether to fully type these legacy v1-style routes or defer behind v1 deprecation work (Phase 6). |
