...
const SLA_PATH = path.join(
  REPO_ROOT,
  "data",
  "attribute-completeness-sla.json",
);
...
for (const row of charsCatRows)
  charsCatMap.set(row.category, Math.trunc(num(row.n)));
...
const latestRecentSnapshot = snapshotRows.find(
  (s) => s.captured_at >= sevenDaysAgo,
);
const lastWeekSnapshot = snapshotRows.find((s) => s.captured_at < sevenDaysAgo);

const currentCoverage = latestRecentSnapshot?.coverage_pct ?? fillRate;
const lastWeekCoverage = lastWeekSnapshot?.coverage_pct;
const velocityDelta =
  lastWeekCoverage === undefined ? null : currentCoverage - lastWeekCoverage;
...
const lines: string[] = [
  ...
  "|----------|-----------|-------------|-----------|",
  ...CATEGORIES.map((cat) => {
    const { null: nullCount, total } = byCategoryBacklog[cat] ?? {
      null: 0,
      total: 0,
    };
...
if (topMisses.length > 0) {
  ...
  lines.push(
    "## SLA Misses",
    "",
    "_No SLA misses detected — all targets met._",
    "",
  );
}
...
if (topNullCategories.length > 0) {
  lines.push(
    ...
    "| Category | Null Cells |",
    "|----------|-----------|",
    ...topNullCategories.map(
      (c) => `| ${c.category} | ${c.null.toLocaleString()} |`,
    ),
    "",
  );
}
...
if (velocityDelta !== null) {
  ...
  if (velocityDelta > 0) direction = "improved";
  else if (velocityDelta < 0) direction = "regressed";
...
console.log(
  `[weekly-report] Null backlog: ${totalNull.toLocaleString()} cells | Fill rate: ${pct(fillRate, 2)} | SLA misses: ${slaMisses.length}`,
);
