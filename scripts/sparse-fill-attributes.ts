...
console.log(
  `[sparse-fill]   ${storedRows.length} non-null cells across ${storedByChar.size} characters`
)
...
for (const [category, catGaps] of grouped) {
  ...
  for (let i = 0; i < catGaps.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      console.error(
        `[sparse-fill]   batch ${i / BATCH_SIZE + 1} FAILED: ${(err as Error).message}`
      )
      continue
    }
    ...
    for (const gap of batch) {
      ...
      for (const key of gap.missingKeys) {
        ...
        if (v === null) continue // skip "unknown" — leave NULL for next round
        filled.push({ characterId: gap.characterId, attributeKey: key, value: v })
      }
    }
    console.log(
...
console.log(
  `\n[sparse-fill] complete: ${filled.length} cells filled out of ${totalPairs} requested  tokens: prompt=${totalPromptTokens} completion=${totalCompletionTokens}`
)
...
for (let i = 0; i < filled.length; i += CHUNK) {
  ...
  const values = chunk.map(
    (c) =>
      `  ('${sqlEscape(c.characterId)}', '${sqlEscape(c.attributeKey)}', ${c.value}, 0.85, '${sqlEscape(evidence)}')`
  )
  insertLines.push(values.join(',\n') + ';')
}
const sqlOut = [...headerLines, ...insertLines, 'COMMIT;'].join('\n')
