...
| MP.6 perf/offline evidence package | 🟡 Pending | CI now blocks on the mobile reliability/perf Vitest slice, but MP.6 closeout still requires device p95 timings and airplane-mode recordings. In-app p95 diagnostics are available on Stats, and the execution steps remain documented in the device validation checklist. | `package.json` (`mobile:reliability-gate`), `.github/workflows/mobile-ci.yml`, `apps/mobile/src/network/mobileOfflineQueue.test.ts`, `apps/mobile/src/screens/StatsScreen.tsx`, `docs/mobile/device-validation-checklist.md` |
