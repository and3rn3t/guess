...
| <a id="dq-9"></a>DQ.9 | **Source-strength weighting per attribute class** | Medium | Empirical study (one-time, 200-character sample): which source is most accurate for which attribute? TMDb wins on movie character ages; AniList on anime moralities; ComicVine on comic-character first-appearance years. Encode as `source_weights[attr_key][source]` in `enrich.ts`. Weighted vote replaces "first source wins." Pairs with EN.11; this is the data behind it. |
| <a id="dq-10"></a>DQ.10 | **Wikidata SPARQL secondary cross-check** | Medium | For each character with a `wikidata_id`, run a SPARQL query asking for the same attributes via structured facts (P31 = instance of, P21 = sex/gender, P509 = cause of death, etc.). Compare to LLM extraction. SPARQL never hallucinates. Costs nothing (Wikidata is free). |
| <a id="dq-11"></a>DQ.11 | **Disagreement triage queue** | Low | New `/admin/data-quality/conflicts` view: every open `agreement_score < 0.6` case, sorted by character popularity × attribute importance. One-click "accept majority", "accept minority + provide evidence", "mark genuinely ambiguous (lock value)". Replaces ad-hoc dispute review. |
...
| <a id="dq-12"></a>DQ.12 | **Image-attribute consistency audit** | Medium | For every character, vision model re-evaluates whether the stored attribute matches the image. `hasGlasses: true` but no glasses visible? Surface in admin as either (a) wrong attribute or (b) wrong image. The "your image and your data disagree" finder. Low-frequency Cron — runs through full catalog over a month. |
| <a id="dq-13"></a>DQ.13 | **Multi-image vision consensus** | Medium | When multiple source URLs exist for a character (TMDb, IGDB, ComicVine all return images), run vision on each independently and majority-vote. Eliminates "the one weird picture where they're not wearing their costume" as a data source. |
| <a id="dq-14"></a>DQ.14 | **Style-of-art classifier** | Low | Vision model tags each image: `live_action | 3d_animation | 2d_animation | comic_art | game_render | photo`. Stored on`characters.image_style`. Powers grid filters, future "guess the medium" game modes, and a quality signal (mismatched art styles in the catalog look bad). |
...
| <a id="dq-25"></a>DQ.25 | **Catalog gap analysis vs. cultural lists** | Low | Cron compares catalog against IMDb Top 250 lead characters, IGN Top 100 video game characters, MyAnimeList top 100, Wikipedia "List of fictional X" pages. Generates `data/catalog-gaps-YYYY-MM.md` weekly. The "obvious omissions" report. Same engine as EN.16. |
| <a id="dq-26"></a>DQ.26 | **Era / decade balance audit** | Low | Distribution of characters by `firstAppearedYear`. If 80% of catalog is post-2000, surfaces in admin as a balance warning. Drives intentional curation decisions. |
...
### Completeness & Closure Program

| # | Item | Effort | Notes |
|---|------|--------|-------|
| <a id="dq-31"></a>DQ.31 | **Definition of complete + release gate** | Medium | Introduce a canonical `data_complete_score` and hard thresholds for production readiness. Minimum gate: global completeness %, per-category completeness %, evidence coverage %, source-id coverage %, and unresolved-high-priority dispute count. Start warn-only for 2 weeks, then fail release checks when below threshold. Primary surfaces: `scripts/data-quality/compute-completeness.ts`, `.github/workflows/ci.yml`, `/api/admin/data-quality`, `/admin/data-quality/completeness`. |
| <a id="dq-32"></a>DQ.32 | **Attribute completeness SLA matrix** | Medium | Add a committed matrix (`data/attribute-completeness-sla.json`) with per-(attribute, category) targets (example: `isHuman` 100%, `firstAppearedYear` 95%, `personality` 70%). Dashboard and cron checks read from one source of truth. Primary surfaces: `data/attribute-completeness-sla.json`, `scripts/data-quality/check-sla.ts`, `/api/admin/data-quality`, `/admin/data-quality/completeness`. |
| <a id="dq-33"></a>DQ.33 | **Deterministic null-closure queue** | Medium | Build a daily queue of missing (character, attribute) pairs prioritized by `popularity × selector_impact × confidence_gap × staleness`. This is the canonical feed for DQ.22 auto-fill and manual review, so high-impact NULLs close first. Primary surfaces: `scripts/data-quality/build-null-closure-queue.ts`, `functions/cron/index.ts`, `functions/api/admin/data-quality/closure-queue.ts`, `/admin/data-quality/completeness`. |
| <a id="dq-34"></a>DQ.34 | **Source-ID completeness guardrail** | Low | Track `% characters with valid source IDs` for TMDb, AniList, IGDB, ComicVine, Wikidata. Daily job verifies referential health (missing, malformed, 404, rate-limited). Broken IDs route to admin repair queue before they silently degrade enrichment quality. Primary surfaces: `scripts/data-quality/check-source-ids.ts`, `functions/cron/index.ts`, `/api/admin/source-health`, `/admin/data-hygiene`. |
| <a id="dq-35"></a>DQ.35 | **Image health completeness guardrail** | Low | Track `% characters with usable portrait` (present, fetchable, non-placeholder, quality above threshold, style not mismatched). Failed rows go to an image-fix queue; DQ.12 audits and EN.20 replacement jobs consume this queue. Primary surfaces: `scripts/data-quality/check-image-health.ts`, `scripts/ingest/enrich.ts`, `/api/admin/image-health`, `/admin/data-hygiene`. |
| <a id="dq-36"></a>DQ.36 | **Manual curator closure queue** | Medium | Introduce a first-class queue for ambiguous/uninferable rows (`cannot_infer`, `canon_conflict`, `subjective`) with assignment, resolution reason, and lock semantics. Prevents endless reprocessing loops for cases automation cannot resolve safely. Primary surfaces: migration for `curation_queue`, `/api/admin/curation-queue`, `/admin/data-quality/completeness`, `functions/api/admin/attribute-disputes/resolve.ts`. |
| <a id="dq-37"></a>DQ.37 | **Risk-tiered revalidation cadence** | Low | Revalidate characters by risk tier instead of flat sampling: Tier 1 (top played + high disagreement) daily, Tier 2 weekly, Tier 3 monthly. Maximizes quality impact per token dollar while improving coverage on the rows players actually see. Primary surfaces: `functions/cron/index.ts`, `scripts/reconcile-attributes.ts`, `scripts/data-quality/select-risk-tier-sample.ts`, `/api/admin/data-quality`. |
| <a id="dq-38"></a>DQ.38 | **Completeness burndown + weekly report** | Low | Add `/admin/data-quality/completeness` burndown for NULL backlog, SLA misses, and queue aging; emit a weekly markdown artifact (`data/quality-reports/quality-YYYY-WW.md`) summarizing closure velocity, blockers, and next highest-impact gaps. Primary surfaces: `/admin/data-quality/completeness`, `/api/admin/data-quality/completeness`, `scripts/data-quality/weekly-report.ts`, `.github/workflows/reconcile-nightly.yml`. |

#### Definition of Complete (canonical gate)

- **Global completeness** = non-null required cells / total required cells.
- **Per-category completeness** = non-null required cells in category / total required cells in category.
- **Evidence coverage** = attributes with non-empty evidence / attributes expected to carry evidence.
- **Source-ID coverage** = characters with all required source IDs present and valid / total characters.
- **Open high-priority disputes** = `attribute_disputes` rows with `status = 'open'` and `confidence >= 0.8`.
- **data_complete_score** = `0.35*global + 0.25*category_floor + 0.2*evidence + 0.1*source_id + 0.1*dispute_health`, where `dispute_health = 1 - min(1, open_high_priority_disputes / dispute_budget)`.
- **Warn gate (first 2 weeks):** CI warns when `data_complete_score < 0.92` OR any category is below its SLA floor.
- **Fail gate (after 2 weeks):** CI fails when `data_complete_score < 0.95`, any category is below SLA floor, or open high-priority disputes exceed budget.
...
- 🟡 **In progress:** [I.9](#i-9) AI Gateway semantic caching 7d cache-hit window (expected: 2026-05-07)
- ✅ **Completed:** [MP.7](#mp-7) release & handoff gate (2026-05-10) · [MP.6](#mp-6) reliability & performance gate (2026-05-10)
...
- 🎯 **Current wave focus:** Wave 5 DQ completeness program ✅ complete; mobile parity execution (MP.1 ✅ 2026-05-07; MP.2 ✅ 2026-05-07; MP.3 ✅ 2026-05-09; MP.4 ✅ 2026-05-09; MP.5 ✅ 2026-05-09; MP.6 ✅ 2026-05-10; MP.7 ✅ 2026-05-10). iOS parity complete.
...
| 🟡 | 8 | [I.9](#i-9) | AI Gateway semantic caching | S | Toggle in CF dashboard. 20–40% LLM cost cut for question generation. Zero code. | AI Gateway dashboard shows ≥20% cache hit rate over a rolling 7-day window. (Shipped Apr 30 via CF dashboard toggle; awaiting 7d verification window — expected 2026-05-07.) |
...
| ✅ 2026-05-01 | 34 | [AN.11](#an-11) | "Aha moment" detector | M | Surfaces which questions consistently produce the posterior jump. The selector's real value. | Each game's reasoning panel highlights the question that produced the largest posterior jump. (Shipped: `posteriorHistory` + `stepTopTen` tracked in `GameSession` and recorded in `answer.ts` after each answer. `computeAhaMoment()` in `functions/api/admin/_aha.ts` (15 tests) finds the argmax jump; `result.ts` writes `aha_attr`/`aha_jump` to `game_stats` via migration `0041_aha_moment.sql`. Nightly aggregator `aggregateAhaMoments()` groups by attribute → count/median/avg and writes to KV `kv:aha-moments`. `GET /api/admin/analytics/aha-moments` exposes the summary. `AnalyticsRoute` renders a top-10 card. `ReasoningPanel` sparkline highlights the breakthrough step with an amber `ReferenceDot` + "⚡ Breakthrough at Q{n}" label. Test count 914 → 929.) |
| ✅ 2026-05-01 | 35 | [AN.21](#an-21) | Catastrophic-failure replay queue | M | Auto-snapshots every game where the player's target wasn't in the engine's top 10. The most actionable training set. | Failure-replay queue auto-snapshots every game where the target wasn't in engine top 10; replayable in admin. (Shipped: `detectCatastrophicFailure()` + `buildStepsJson()` in `functions/api/admin/_triage.ts` (15 tests). `result.ts` writes a row to `triage_queue` (migration `0042_triage_queue.sql`) via `context.waitUntil` on failure games where the actual character never appeared in the per-step top-10. `GET /api/admin/triage` (list + detail by `?id=N`) serves the queue. `TriageRoute` renders expandable rows with a per-step `StepReplay` view (top-10 ranked candidates, actual character highlighted in amber), paginated 50/page. Sidebar nav gains "Failure Triage" under Pipeline. e2e smoke sweep updated.) |
...
**Data completeness push (next highest leverage):** DQ.31 definition-of-complete gate · DQ.32 SLA matrix · DQ.33 null-closure queue · DQ.34 source-ID completeness · DQ.35 image health completeness · DQ.36 curator closure queue · DQ.37 risk-tiered revalidation cadence · DQ.38 weekly burndown report

#### Wave 5 Completeness Program Queue
...
| **Deprecate v1 KV endpoints** ✅ 2026-05-03 | `functions/api/{characters,questions,corrections,stats,sync}.ts` — legacy KV-backed endpoints from before D1. All five handlers now emit `Deprecation: true` + `Sunset: Wed, 01 Jan 2027 00:00:00 GMT` response headers on every GET. Plan a cleanup migration to drop the handlers after sunset. |
...
| ✅ | <a id="mp-6"></a>MP.6 | **Reliability & performance gate** | M | L3 | All 10+ features meet <100ms tap-to-feedback, <150ms transition start, offline-first UX, network resilience. ✅ 2026-05-10 |
| ✅ | <a id=\"mp-7\"></a>MP.7 | **Release & handoff gate** | S | L2+ | Final parity validation, documentation handoff to Xcode team, appstore submission prep. ✅ 2026-05-10 |
...
| <a id="dx-18"></a>DX.18 | **Coverage diff in PR comments** | Low | ✅ 2026-05-03 — `checks-tests` in `.github/workflows/ci.yml` now posts per-PR Vitest coverage comments via pinned `davelosert/vitest-coverage-report-action` (same-repo PRs), with required `pull-requests: write` permission. `vitest.config.ts` now emits `json-summary` + `json` alongside text/lcov reporters so coverage deltas are machine-readable. |
...
| <a id="dx-23"></a>DX.23 | **`pnpm doctor` health check** | Low | ✅ 2026-05-03 — `scripts/doctor.ts` checks Node ≥ 22, pnpm version, `wrangler whoami`, `.dev.vars` + `.env` required keys, `node_modules`, gitleaks (warn). `.dev.vars.example` added as source-of-truth for required keys. Script verified: `pnpm doctor` runs cleanly with green/red checks working. |
...
| <a id="dx-25"></a>DX.25 | **Wrangler tail with structured grep** | Low | ✅ 2026-05-03 — `scripts/tail.ts` wraps `wrangler pages deployment tail guess --format=json` (Pages-correct command). Supports `--env=production|preview` (default: preview), `--filter=status>=400`, `--filter=path~<regex>`. Colored output, header redaction for Cookie/Authorization. Script verified working. |
...
| <a id="dx-27"></a>DX.27 | **VS Code workspace settings + recommended extensions** | Low | ✅ 2026-05-03 — `.vscode/tasks.json` (validate as default build task Ctrl+Shift+B, dev/cf:dev/test:watch as background tasks), `.vscode/launch.json` (tsx script debugger + wrangler attach), `extensions.json` +6 (vitest.explorer, ms-playwright.playwright, github.vscode-github-actions, mikestead.dotenv, tamasfe.even-better-toml, aaron-bond.better-comments), `settings.json` +fileNesting/inlayHints/linkedEditing/vitest config. All files verified present. |
...
| <a id="dx-30"></a>DX.30 | **Generated D1 types from schema** | Low | ✅ 2026-05-03 — committed type generation pipeline via `pnpm db:types` (`scripts/generate-db-types.ts`) emitting `functions/api/_db-types.ts` from ordered `migrations/*.sql` (`CREATE TABLE` + `ALTER TABLE ... ADD COLUMN` support). Worker handlers import these row interfaces for compile-time column checks. CI `db-checks` now enforces zero drift by regenerating and failing if `functions/api/_db-types.ts` is stale. |
| <a id="dx-31"></a>DX.31 | **OpenAPI spec auto-derived from handlers** | Medium | ✅ 2026-05-03 — deterministic OpenAPI artifacts are now generated from handlers (`docs/openapi.{json,yaml}`, `docs/openapi-inventory.json`, `public/openapi.yaml`) with validate/drift scripts and `/admin/api-docs` scaffold. Coverage now includes explicit request/response schemas across v2 endpoints, all discovered `/api/admin/*` operations (including long-tail enrichment/hygiene/duplicates/recommender/resolve-stack flows), and legacy `/api/*` routes (`characters`, `corrections`, `csp-report`, `images`, `llm`, `llm-stream`, `questions`, `stats`, `sync`), reducing generic 200 JSON object contracts to zero. |
...
| <a id="dx-38"></a>DX.38 | **Conventional commit linter** | Low | ✅ 2026-05-03 — `commitlint.config.mjs` + `.husky/commit-msg` hook with shell guard that skips Merge/Revert/fixup!/squash! auto-messages so rebases are never blocked. `@commitlint/cli` + `@commitlint/config-conventional` added as devDeps. Hook verified in place and enforcing conventional commits. |
...
| EN.4 | **Golden character regression set** | Canonical owner moved to [DQ.1](#dq-1). Enrichment scope here is implementation-only: keep prompt-template replay harnesses aligned with the golden gate and ensure enrichment prompt changes always run through the same CI quality gate. |
...
| EN.6 | **Per-attribute reliability score** | Canonical owner moved to [DQ.3](#dq-3) and [DQ.9](#dq-9). Enrichment scope here is source-specific extraction instrumentation and logging that feeds the reliability model, not a separate quality policy. |
| EN.7 | **Schema drift detector** | Canonical owner moved to [DQ.21](#dq-21). Enrichment scope here is keeping the prompt-schema generator and attribute extraction code path compatible with the shared drift checks. |

### Confidence, Provenance & Self-Healing

| # | Item | Notes |
|---|------|-------|
| EN.8 | **Provenance trail per attribute value** | Canonical owner moved to [DQ.28](#dq-28). Enrichment scope here is improving evidence extraction quality and source-quote fidelity in pipeline writes. |
| EN.9 | **Calibrated confidence via player corroboration** | Canonical owner moved to [DQ.5](#dq-5). Enrichment scope here is calibrating confidence update formulas that consume corroboration outputs. |
| EN.10 | **Self-healing for invalid combinations** | Canonical owner moved to [DQ.4](#dq-4). Enrichment scope here is post-processor implementation and constrained retry prompts. |
| EN.11 | **Source-attribution-aware ingestion** | Canonical owner moved to [DQ.9](#dq-9) and [DQ.10](#dq-10). Enrichment scope here is capture/storage of per-source evidence needed for weighted resolution. |
| EN.12 | **Counter-factual probe set** | Canonical owner moved to [DQ.19](#dq-19). Enrichment scope here is probe-prompt generation and batch execution ergonomics. |
...
| EN.16 | **Catalog gap analysis vs. external lists** | Canonical owner moved to [DQ.25](#dq-25). Enrichment scope here is ingestion candidate generation from the gap report output. |
| EN.17 | **Era / decade balance audit** | Canonical owner moved to [DQ.26](#dq-26). Enrichment scope here is using balance outputs to shape discovery/enrichment batches. |

### Multimodal & Vision

| # | Item | Notes |
|---|------|-------|
| EN.18 | **Vision-derived visual attributes** | Canonical owner moved to [DQ.2](#dq-2) and [DQ.12](#dq-12). Enrichment scope here is model routing and throughput optimization for large batch visual extraction runs. |
...
| ✅ 2026-05-02 AP.6 | **Stale-data freshness badges** | Every data card / table shows a "fetched 2m ago" pill (subtle, top-right). Stale > 5min = amber, > 30min = red. Click to refetch. Cheap signal that you're not staring at cached numbers. |
...
| ✅ 2026-05-03 AP.10 | **Global command palette (`⌘K`)** | `cmdk` is already a dep. Bind `⌘K` in admin shell to a palette: jump to any route, jump to any character by name (FTS-powered), recent admin actions, common commands ("export game_stats CSV", "rotate basic-auth"). Cuts navigation latency to near-zero. (Shipped: ⌘K keybind in AdminShell + enhanced `AdminCommandPalette` component. Features: (1) Route sections filtered by search, (2) Character search with FTS (limit 5 results), (3) Common commands (Export CSV, View Errors, Check Quality), (4) All sections dynamically filter based on search input. 14 unit tests cover filtering logic — character matching, case-insensitivity, section filtering, command filtering. Ready to wire into remaining admin UX flows.) |
| ✅ 2026-05-02 AP.11 | **Breadcrumbs + page titles** | Every route sets `document.title` to `Admin · {route name} · Andernator` so browser tabs are distinguishable. Inline breadcrumb (`Admin / Pipeline / Proposed Attributes`) replaces the current title-only header. |
...
| ✅ 2026-05-03 AP.14 | **CSV / JSON export buttons on every table** | One `<TableExportMenu />` primitive (Copy as TSV / Download CSV / Download JSON / Copy as Markdown table). Wired to whatever rows are currently filtered. Removes the constant temptation to drop into D1 SQL for one-off exports. (Shipped: `src/components/admin/TableExportMenu.tsx` exports 4 pure helpers — `rowsToTsv`, `rowsToCsv`, `rowsToJson`, `rowsToMarkdown` — with RFC 4180 CSV escaping, null handling, and format-specific escape rules. Component UI: 4 buttons (Copy TSV / Markdown, Download CSV / JSON) with 2s "copied" feedback via Check icon. 15 comprehensive unit tests cover edge cases — commas/quotes/newlines/tabs/pipes, type preservation, empty rows. Disabled when rows.length === 0. Ready to wire into Characters, Questions, Disputes tables.) |
...
| ✅ 2026-05-02 AP.17 | **Toast feedback standardization** | Mix of `toast.success`, alert dialogs, and silent reloads today. Standard: every mutation shows a toast with action result + Undo button (where reversible) + "View" link to the affected row. `sonner` already wired. |
...
| ✅ 2026-05-03 AP.22 | **`/admin/about` build & data freshness card** | Single card showing: deployed commit SHA + timestamp, current D1 schema version (latest applied migration), KV namespace IDs, last enrichment run, last cron run, last D1 backup, app version (`package.json`). The "what am I looking at?" page when something's weird. (Shipped: `GET /api/admin/about` endpoint returns `{ appVersion, schemaVersion, lastEnrichmentRun: {timestamp, batchId}, lastCronRun: {timestamp, name}, lastD1Backup: {timestamp} }`. UI component `AboutRoute.tsx` displays 3 cards (Application / Data Freshness / optional KV Namespaces) with relative time helpers ("42d ago", "just now", etc.). Integrated into AdminShell navigation + smoke-test suite. Passes full validation.) |
...
| 2026-04 | No auth-backed global identity leaderboard (yet) | Requires auth; complexity not worth it without an audience. Daily anonymous leaderboard is already shipped as a separate scope |
...
| 2026-05 | Admin recommender/hygiene calls moved server-side | Category/attribute recommender and hygiene scans were doing browser-originated LLM orchestration, which made auth/env handling brittle and blocked centralized validation/logging. Added dedicated `/api/admin/*` endpoints (recommender + hygiene + costs) with zod input/output guards and round-trip tests, then switched admin UI callers to typed server client modules. This keeps OpenAI usage and failure handling on the server boundary and makes route-level regressions testable in CI. |
| 2026-05 | Keep `SCORE_MATCH=1.0`; tune remaining scoring weights | A stronger unconstrained simulation candidate (`SCORE_MATCH=0.8`) improved short-run metrics but violated existing invariants and failed tests. Adopted the best constrained candidate from a 40x40 sweep (`mismatch=0.03`, `maybe=0.8`, `maybeMiss=0.4`) so selector assumptions remain intact while improving against the prior constants. Regression gate and full validate remained green. |
