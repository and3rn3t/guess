...
- If `strict_readiness_win_pct` is low and `forced_guess_rate` is low, raise the readiness bar in [functions/api/v2/_game-engine.ts](../functions/api/v2/_game-engine.ts) and [packages/game-engine](../packages/game-engine).
